 ...
 * Uses Amazon Polly via /api/sonic/speak for voice output (TTS).
 * Falls back to browser TTS when Polly is unavailable.
 */
...
export class VoiceCapture {
     ...
     * Tries Amazon Polly first, falls back to browser TTS.
     */
    async speak(text: string): Promise<void> {
        this.setStatus('speaking');

        try {
            // Try Polly (backend handles priority)
            const sonicResult = await speakWithSonic(text);

            if (sonicResult.audioBase64 && !sonicResult.mock) {
                // Play audio (MP3 from Polly)
