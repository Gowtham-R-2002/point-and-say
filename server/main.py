...
from services.sonic import create_sonic_session
...
# Persistent sonic session (reused across requests)
_sonic_session = None


def _get_sonic_session():
    global _sonic_session
    if _sonic_session is None:
        _sonic_session = create_sonic_session()
    return _sonic_session
...
async def sonic_transcribe(req: TranscribeRequest):
    ...
    Returns transcription text and optionally audio response.
    Falls back gracefully if Sonic is unavailable.
    """
    log("🎤", "SONIC TRANSCRIBE", audioLength=len(req.audioBase64)[:50] + "...")

    session = _get_sonic_session()
    result = await session.transcribe_audio(req.audioBase64)
...
async def sonic_speak(req: SpeakRequest):
    ...
    Priority: Amazon Polly (neural) → Nova Sonic → fallback.
    Returns base64-encoded audio.
    """
    log("🔊", "TTS SPEAK", text=req.text[:100])

    # Try Amazon Polly first (much better quality)
    try:
    ...
    # Fallback: Nova Sonic
    session = _get_sonic_session()
    audio = await session.generate_speech(req.text)

    if audio:
        log("✅", "SONIC AUDIO", audioLength=len(audio))
        return {"status": "ok", "audioBase64": audio, "format": "audio/wav", "engine": "sonic", "mock": False}
    else:
        log("⚠️", "TTS FALLBACK", reason="No audio from Polly or Sonic")
